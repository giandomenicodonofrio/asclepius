...
class EnsembleModel:
    ...
    def _custom_train(self, combination, X_pointer, y_pointer, train_idx, val_idx, epochs, batch_size=64):
    ...
    def fit(self, X_pointer, y_pointer, train_idx, val_idx, epochs, batch_size=64):
        trainer = ConditionalParallelTrainer(train_function=self._custom_train, parallel_training=True, n_workers=3)
        args = []
        for combination in self.lead_configuration:
            args.append((combination, X_pointer, y_pointer, train_idx, val_idx, epochs, batch_size))
        trainer(*args)
